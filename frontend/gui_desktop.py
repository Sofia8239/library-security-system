...
class SessionManager:
    ...
    def _sign(self, data: bytes) -> str:
        digest = hmac.new(self.key.encode('utf-8'), data, hashlib.sha256).digest()
        return base64.urlsafe_b64encode(digest).decode('utf-8')

    def create_session(self, username, remember_me=False):
        import time
        expiry = int(time.time()) + (30 * 24 * 60 * 60) if remember_me else int(time.time()) + (24 * 60 * 60)
        token_data = f"{username}:{expiry}".encode('utf-8')
        signature = self._sign(token_data)
        token = base64.urlsafe_b64encode(token_data).decode('utf-8') + '.' + signature
    ...
    def load_session(self):
        ...
        try:
            ...
            username, expiry = payload.decode('utf-8').split(':')
